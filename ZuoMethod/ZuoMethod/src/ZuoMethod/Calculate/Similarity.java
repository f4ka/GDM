...
public class Similarity {
    ...
    public Double[] Similarity(Double[][] Trapezes,Double GravityCenters[], int NumberTrapezes)
    {
        ...
        {
        ZuoMethod.SimilarityDegree[i] = Math.pow((1 - (SumPairs[i]/4)),Exponents[i]);
        ...
        ZuoMethod.SimilarityDegree[i] = ZuoMethod.SimilarityDegree[i] * (num/den);
        }
        return ZuoMethod.SimilarityDegree;
    }
    ...
    public boolean CheckWeights(Double[] WeightsDecisors)
    {
        boolean x=false;
        for(int i=0;i<WeightsDecisors.length;i++)
        {
            if(WeightsDecisors[i]>1)
                x=true;
        }
        return x;
    }
    
    public Double[] NormalizationWeights(Double[] WeightsDecisors)
    {
        for(int i=0;i<WeightsDecisors.length;i++)
        {
            WeightsDecisors[i] = WeightsDecisors[i]/Maximum;
        }
        return WeightsDecisors;
    }    
}
